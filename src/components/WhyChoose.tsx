...
export default function WhyChoose() {
  ...
  return (
    <section ref={ref} className="wc-section" style={{ background: "#fff", padding: "80px 60px" }}>
      <div style={{ maxWidth: 1400, margin: "0 auto" }}>
        ...
        <div className="wc-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 32 }}>
          {REASONS.map((r, i) => {
            const Icon = r.icon;
            return (
              <motion.div
                ...
                style={{ padding: "40px 32px", background: "#F8F9FA", borderRadius: 8, border: "1px solid #E5E7EB" }}
              >
                <div style={{ marginBottom: 24, height: 40, display: "flex", alignItems: "flex-start" }}>
                  <Icon size={32} strokeWidth={1.5} style={{ color: "#0066FF" }} />
                </div>
                <h3 style={{ fontSize: 19, fontWeight: 700, color: "#0A1929", marginBottom: 14, letterSpacing: "-0.01em" }}>{r.title}</h3>
      ...
      <style>{`
        @media(max-width:1024px){.wc-section{padding:60px 40px!important}.wc-grid{grid-template-columns:1fr 1fr!important;gap:24px!important}}
        @media(max-width:768px){.wc-section{padding:48px 24px!important}.wc-grid{grid-template-columns:1fr!important}.wc-header h2{font-size:32px!important}.wc-header p{font-size:14px!important}}
      `}</style>
    </section>
  );
}
